...
module LoopFilter #(
  ...
  always @(DYNAMIC_VAL or reset_i or kp_i or ki_i) begin
    if (DYNAMIC_VAL) begin
      kp_x = kp_i;
      ki_x = ki_i;
    end else begin
      kp_x = KP;
  ...
  always @(posedge gen_clk_i or posedge reset_i) begin
  ...
  assign ki_error_resize_c = $signed(
      ki_error_inte_c[KI_ACCUM_OVERHEAD+KI_MULT_RES_WIDTH-1:KI_ACCUM_OVERHEAD]
  );
