...
module RingOsc #(
  ...
  genvar i;
  generate
    for (i = 1; i <= INVERTERNUM; i = i + 1) begin : RING

      always @(ringwire_c[i-1]) begin
        #10e-3 ring_wire_c[i] = ringwire_c[i-1];
      end
  ...
  always @(freq_sel_i, ringwire_c, reset_i) begin
    if (reset_i) f_sel_mux_out_r = 1'b0;
    else if (freq_sel_i == {(CTRL_WIDTH) {1'b0}}) f_sel_mux_out_r = ringwire_c[INVERTERNUM];
    else f_sel_mux_out_r = ringwire_c[INVERTERNUM-2*freq_sel_i];
  end
