...
module StateMachine (
  ...
  always @(posedge fpga_clk_i) begin
    if (reset_i) current_state_r <= WAITING;
    else current_state_r <= next_state_r;
  end

  always @(current_state_r, ref_pos_edge_x, gen_pos_edge_x, counter_cleared_i) begin
    case (current_state_r)
      WAITING:
      //if both edges occur then COPY_CLEAR
      if (ref_pos_edge_x && gen_pos_edge_x)
        next_state_r = COPY_CLEAR;
      ...
      //otherwise stay here 
        ...
        next_state_r = GEN_FIRST;
      COPY_CLEAR:
      //if cleared move to wait state
      if (counter_cleared_i)
        next_state_r = WAITING;
